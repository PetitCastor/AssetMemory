...
internal static class TrayApp
{
    ...
    /// <summary>Loads the tray icon from the embedded resource, falling back to the stock Windows
    /// app icon if it's somehow missing (should never happen -- it's baked into the assembly).</summary>
    private static Icon LoadIcon()
    {
        using var stream = Assembly.GetExecutingAssembly().GetManifestResourceStream("app-icon.ico");
